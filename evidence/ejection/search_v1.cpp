...
std::string state_key(const State& state) {
    std::string key;
    key.reserve(state.bits().size());
    for (const std::uint8_t bit : state.bits()) {
...
void run_ejection_worker(
    ...
    auto print_metrics = [&]() {
        ...
        std::cerr
            ...
            << " max_origin_distance=" << maximum_origin_distance
    ...
    for (std::uint64_t chain = 0;
        ...
        for (int depth = 0;
            ...
            for (const EjectionCandidate& candidate : candidates) {
                ...
                const std::string key = state_key(child);
                if (!seen_states.insert(key).second) {
                    continue;
                }
                generated_state = true;
                ++beam_states;
                const int origin_distance =
                    bit_distance(child.bits(), origin_bits);
                maximum_origin_distance = std::max(
                    maximum_origin_distance,
                    origin_distance
                );
                ...
                if (origin_distance >= 9 && better_distant) {
                    best_distant_state = child;
                    best_distant_distance = origin_distance;
                }
                if (child.uncovered() <= 6 && origin_distance >= 9) {
                    ++distant_six_gap_visits;
                    if (
                        distant_six_gap_states.find(key) ==
                        ...
                        if (
                            ...
                            distant_six_gap_states.insert(key);
                        } else {
