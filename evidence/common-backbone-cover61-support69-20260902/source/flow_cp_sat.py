...
def build_flow_model(
    *,
    ...
    add_cyclic_weight_path_constraints: bool = False,
    ...
    if not isinstance(add_cyclic_weight_path_constraints, bool):
        raise ValueError(
            "cyclic weight path constraint option must be a Boolean"
        )
    ...
    if radius == 1 and add_layer_constraints:
        ...
        add_path_constraints = (
            ...
            or add_cyclic_weight_path_constraints
        )
        # One cyclic component gives a closed nearest-neighbor weight walk.
        if n >= 3 and add_path_constraints:
            multiplicity_layers = [
                sum(
                    count[word]
                    for word in range(word_count)
