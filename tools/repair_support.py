...
def solve_support_repair(
    *,
    ...
    distinct_windows: bool = True,
    support_size: int | None = None,
    duplicate_edge: int | None = None,
    duplicate_kind: str = "any",
    duplicate_scope: str = "any",
) -> tuple[dict[str, Any], list[int] | None]:
    support = validate_support(support, n=n)
    if minimum_overlap is not None and not 0 <= minimum_overlap <= len(
        support
    ):
        raise ValueError(
            "minimum overlap must be between zero and the reference size"
        )
    if exact_overlap is not None and not 0 <= exact_overlap <= len(support):
        raise ValueError(
            "exact overlap must be between zero and the reference size"
        )
    if minimum_overlap is not None and exact_overlap is not None:
        raise ValueError("minimum overlap and exact overlap are exclusive")
    if maximize_overlap and exact_overlap is not None:
        raise ValueError("maximize overlap and exact overlap are exclusive")
    ...
    if duplicate_scope not in {"any", "reference", "outside"}:
        raise ValueError(
            "duplicate scope must be any, reference, or outside"
        )
    ...
    support_set = set(support)
    if duplicate_scope != "any":
        if artifacts.extra is None:
            raise ValueError(
                "duplicate scope requires an exact one-repeat stage"
            )
        repeated_in_reference = sum(
            artifacts.extra[word] for word in support
        )
        artifacts.model.add(
            repeated_in_reference
            == int(duplicate_scope == "reference")
        )
    complete_reference = len(support) == length
    for word in range(1 << n):
        if word in support_set:
            artifacts.model.add_hint(artifacts.use[word], 1)
            artifacts.model.add_hint(artifacts.count[word], 1)
        elif complete_reference:
            artifacts.model.add_hint(artifacts.use[word], 0)
            artifacts.model.add_hint(artifacts.count[word], 0)
    ...
    if exact_overlap is not None:
        artifacts.model.add(overlap == exact_overlap)
    if maximize_overlap:
    ...
    summary: dict[str, Any] = {
        ...
        "repeated_windows_allowed": not distinct_windows,
        ...
        "maximum_reference_omissions": (
            None
            if minimum_overlap is None
            else len(support) - minimum_overlap
        ),
        "maximum_replacements": (
            None
            if minimum_overlap is None or not complete_reference
    ...
    selected = [solver.value(variable) for variable in artifacts.use]
    ...
    reference_overlap = sum(selected[word] for word in support)
    reference_occurrences = sum(counts[word] for word in support)
    summary.update(
        {
            "valid": True,
            "overlap": reference_overlap,
            "reference_edge_occurrences": reference_occurrences,
            "reference_edges_omitted": len(support) - reference_overlap,
            "selected_outside_reference": sum(
                selected[word]
                for word in range(1 << n)
                if word not in support_set
            ),
            "outside_reference_occurrences": (
                length - reference_occurrences
            ),
            "selected_edges": [
                word
                for word, is_selected in enumerate(selected)
                if is_selected
            ],
        }
    )
    if complete_reference:
        summary["replacements"] = len(support) - reference_overlap
    return summary, bits
...
def main() -> int:
    ...
    parser.add_argument("--exact-overlap", type=int)
