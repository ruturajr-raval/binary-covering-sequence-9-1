...
def run_case(
    *,
    ...
    if support_size is not None:
        command.extend(["--support-size", str(support_size)])
    if duplicate_edge is not None:
        command.extend(["--duplicate-edge", str(duplicate_edge)])
    if duplicate_kind != "any":
        command.extend(["--duplicate-kind", duplicate_kind])
    if duplicate_scope != "any":
        command.extend(["--duplicate-scope", duplicate_scope])
    ...
    if status in {"OPTIMAL", "FEASIBLE"}:
        ...
        counts = sequence_window_counts(bits, n)
        repeated_edges = [
            edge
        ...
        actual_duplicate = (
            repeated_edges[0]
            if repeat_excess == 1 and len(repeated_edges) == 1
            else None
        )
        duplicate_is_loop = (
            actual_duplicate is not None
            and actual_duplicate >> 1
            == actual_duplicate & ((1 << (n - 1)) - 1)
        )
        ...
        checks = {
            ...
            "duplicate_edge": (
                duplicate_edge is None
                or actual_duplicate == duplicate_edge
            ),
            "duplicate_kind": (
                duplicate_kind == "any"
                or (
                    actual_duplicate is not None
                    and duplicate_is_loop
                    == (duplicate_kind == "loop")
                )
            ),
            "duplicate_scope": (
                ...
                or (
                    actual_duplicate is not None
                    and (actual_duplicate in support_edges)
...
def main() -> int:
    ...
    summary: dict[str, Any] = {
        "cases": [],
        "candidate_support_size": args.support_size,
